...
namespace Snake_Dekhtiarenko
{
    public partial class Form1 : Form
    {
        ...
        string name;
        private int timer = 0;
        static int nodeHeight = 16;
        static int nodeWidth = 16;
        ...
        public Form1()
        {
            ...
            //Image myimage = new Bitmap(@"D:\Screenshots\PhotoFunia-1496587877.jpg");
        ...
        private void Update(object sender, EventArgs e)
        {
            ...
            if (gameover)
            {
                ...
                {
                    gameTimer.Enabled = false;
                    EnterName dialogBox = new EnterName();
                    dialogBox.ShowDialog();
        ...
        private void Draw(Graphics canvas)
        {
            ...
            if (gameover)
            {
                ...
                canvas.DrawString(score.ToString(), font, Brushes.Purple, new PointF(560, 400));
                font = new Font("Rockwell Condensed", 20.0f,
                    font.Style, font.Unit,
                    font.GdiCharSet, font.GdiVerticalFont);
                canvas.DrawString("Press Enter to Begin New Game", font, Brushes.Gray, new PointF(285, 700));
            }
            else
            {
                if (Pause)
        ...
        private void GameOver()
        {
            ...
            // Append text to an existing file named "WriteLines.txt".
            using (StreamWriter outputFile =
                new StreamWriter(System.IO.Directory.GetCurrentDirectory() + @"\Data.txt"))
            {
                //string s = score.ToString();
                outputFile.WriteLine(name + ":" + score);
            }
            gameTimer.Interval = 100;
        }
        ...
        {
            using (StreamReader fs = new StreamReader(System.IO.Directory.GetCurrentDirectory() + @"\Data2.txt"))
            {
                string s = "";
                while (s != null)
                {
                    s = fs.ReadLine();
                    string[] tmp = s.Split(':');
                    //highScores.Add(Convert.ToInt32(tmp[0]));
                }
            }
            highScores.Sort();
            //highScore = highScores[highScores.Count() - 1];
        }
        ...
        private void highScoresToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                dialogBox.ScoresList.Items.Add(highScores[i]);
            }
